...
use sea_orm::{
    sea_query::{ArrayType, ColumnType, Value, ValueType, ValueTypeErr, Nullable},
            ...
            fn from(v: $wrapper_name) -> Self {
                ...
                match BigDecimal::from_str(&str_val) {
                    ...
                    Err(_) => {
                        ...
                        panic!("Failed to convert {} to BigDecimal: {}", stringify!($inner_type), str_val)
                    }
                }
            }
        }
...
mod tests {
    ...
    fn test_u256_large_value() {
        ...
        let val = DbU256(U256::from_str_radix(
